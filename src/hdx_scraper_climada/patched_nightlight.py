...
https://github.com/CLIMADA-project/climada_python/blob/main/climada/entity/exposures/litpop/nightlight.py
...
def load_nasa_nl_shape(geometry, year, data_dir=SYSTEM_DIR, dtype="float32"):
    ...
    years_available = [
        year.int() for year in CONFIG.exposures.litpop.nightlights.blackmarble_years.list()
    ]
...
def untar_noaa_stable_nightlight(f_tar_ini):
    ...
    with tarfile.open(f_tar_ini) as tar_file:
        extract_name = [
            name for name in tar_file.getnames() if name.endswith("stable_lights.avg_vis.tif.gz")
        ]
        ...
        if len(extract_name) > 1:
            LOGGER.warning(
                "found more than one potential intensity file in %s %s", f_tar_ini, extract_name
            )
...
def load_nightlight_noaa(ref_year=2013, sat_name=None):
    ...
    if sat_name is None:
        fn_light = str(SYSTEM_DIR.joinpath("*" + str(ref_year) + "*.stable_lights.avg_vis"))
    else:
        fn_light = str(SYSTEM_DIR.joinpath(sat_name + str(ref_year) + "*.stable_lights.avg_vis"))
