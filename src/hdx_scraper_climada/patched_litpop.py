...
class LitPop(Exposures):
    ...
    def from_countries(
        ...
        countries_in = [country for lp, country in zip(litpop_list, countries) if lp is not None]
        countries_out = [country for lp, country in zip(litpop_list, countries) if lp is None]
        if not countries_in:
    ...
    def from_shape_and_countries(
        ...
        if isinstance(shape, Shape):
            ...
            shape_gdf = shape_gdf.drop(columns=shape_gdf.columns[shape_gdf.columns != "geometry"])
            ...
            for shp in shape:
                if isinstance(shp, (shapely.geometry.MultiPolygon, shapely.geometry.Polygon)):
                    gdf = gdf._append(exp.gdf.loc[exp.gdf.geometry.within(shp)])
                else:
    ...
    def _from_country(
        ...
        for idx, polygon in enumerate(country_geometry.geoms):
            ...
            if gdf_tmp is None:
                LOGGER.debug(f"Skipping polygon with index {idx} for" + f" country {iso3a}.")
                continue
...
def _get_total_value_per_country(cntry_iso3a, fin_mode, reference_year):
    ...
    if fin_mode == "pc":
        return u_fin.world_bank_wealth_account(cntry_iso3a, reference_year, no_land=True)[1]
        # here, total_asset_val is Produced Capital "pc"
        # no_land=True returns value w/o the mark-up of 24% for land value
    if fin_mode == "pc_land":
        return u_fin.world_bank_wealth_account(cntry_iso3a, reference_year, no_land=False)[1]
    ...
    if fin_mode in ("nfw", "tw"):
        wealthtogdp_factor = u_fin.wealth2gdp(cntry_iso3a, fin_mode == "nfw", reference_year)[1]
...
def gridpoints_core_calc(data_arrays, offsets=None, exponents=None, total_val_rescale=None):
    """
